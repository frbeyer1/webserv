...
std::string Response::_POSTmethod(HttpRequest &request, Server &server)
{
    ...
    _data_location = server.getRoot() + request.getPath();
    struct stat sb;
    if(stat(_data_location.c_str(), &sb) == 0 && S_ISDIR(sb.st_mode))
    {
    ...
    {   
        ...
        if(stat(_data_location.c_str(), &sb) == 0)
        {
        // check allowed methods
        // getMimeType?
    ...
    // how does it work when user wants access to path again
    // messages -> "successfull upload, saving, etc..."
    std::cout << request.getBody() << std::endl;
    _contentPath = "docs/submit_success.html";//-----------------how to handle this
    _contentType = getMimeType(_contentPath);
    _code = checkContent();
    if(_code != 200)
        return(_buildDefaultErrorPage(_code));
